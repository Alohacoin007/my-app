...
-- withdrawal request cannot even be INSERTED unless:
--   (1) the session is AAL2 (the user just passed TOTP 2FA), AND
--   (2) the destination address is on the caller's allowlist AND active (added ≥24h ago).
--
-- Invariant: an on-chain crypto withdrawal request exists ⟹ AAL2 session ⟹ dest ∈
--   active allowlist. Non-crypto (bank/card/wire) withdrawals are untouched; internal
--   Alpexa→Alpexa sends don't create requests, so they're untouched too.
...
begin
  ...
  -- (1) 2FA: the session must be AAL2 (user passed TOTP). Supabase sets the 'aal'
  --     JWT claim to 'aal2' after mfa.verify. Not aal2 → reject (client can't fake it).
  if coalesce(auth.jwt() ->> 'aal', 'aal1') <> 'aal2' then
    raise exception 'Two-factor verification is required to withdraw to an external address.'
      using errcode = 'check_violation';
  end if;

  -- (2) Allowlist: the destination must be on THIS customer's allowlist and active
  --     (added ≥24h ago). Match on address (chain-unique); network label is cosmetic.
  if not exists (
    ...
    raise exception 'This address is not on your active allowlist (add it and wait 24h).'
      using errcode = 'check_violation';
  end if;
