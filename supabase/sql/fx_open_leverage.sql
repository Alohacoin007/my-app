-- Alpexa — fx_open leverage-aware OVERLOAD (optional; deploy AFTER fx_open_margin.sql).
-- ============================================================================
-- Adds a 5-arg fx_open(..., p_leverage) that coexists with the 4-arg version. The client
-- passes the leverage the user picked in the Order dialog; the server CLAMPS it to the
-- house cap:  effective = LEAST(house_cap[cls], p_leverage).
-- → a client can only choose a MORE conservative (lower) leverage, never exceed the cap.
--   Passing 1:500 when the house FX cap is 1:100 still margins at 1:100 (safe). The margin
--   gate + entry price + idempotency are identical to fx_open_margin.sql.
-- Reuses fx_ccy_to_usd / fx_notional_usd / fx_lev_cap from fx_open_margin.sql.
-- Balance is untouched at open (banked at fx_close). Invariant unchanged.
-- ============================================================================
create or replace function public.fx_open(
  p_local_id text, p_symbol text, p_side text, p_size numeric, p_leverage numeric
...
begin
  ...
  v_lev := least(v_cap, greatest(1, coalesce(p_leverage, v_cap)));   -- never exceed the house cap
...
-- test:
--   select public.fx_open('t-'||floor(random()*1e6)::text,'EURUSD','BUY',0.01, 100);
