...
Deno.serve(async (req) => {
  ...
  // B8 (deferred): currently FAIL-OPEN to match production — if CRON_SECRET is set the
  // token is checked, else it runs. To close B8, switch to fail-closed (return 503 when
  // !CRON_SECRET) AND set the secret + add ?token= to the cron (cron_secure.sql).
