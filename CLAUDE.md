...
1. **소스부터 추적 (증상 근처 금지).** "이 화면은 데이터를 **어디서** 읽나?" → 스포츠 앱 = **`live_games` 테이블** ← `sports-games` Edge ← **ESPN ∪ The Odds API** (2026-09-17: ESPN 경기 + ESPN 이 모르는 Odds 이벤트 `LG_o<oddsId>`; 정산 스코어도 ESPN → Odds `scores` 2차, `_o` 는 Odds 전용·void 금지). 클라 ESPN fetch는 **폴백(죽은 경로)**. 증상 옆 코드를 고치지 말고 **소스**를 고친다.
...
- 🧭 **배포 판별 `tests/odds-assign-live.js`** (2026-09-14 신설, 네트워크·아침 점검): live_games 가격이 OLD(teamMatch 배정) 와 NEW(sideOf+가드 95%) 중 어느 코드의 산출인지 라이브로 대조. 소스 락스텝 핀은 **리포 코드**만 보고 **배포된 코드**는 못 본다 — Edge 수동 붙여넣기 배포는 "안 됨/옛 버전" 이 조용히 일어나는 경로라 독립 출처로 확인. 갈리는 경기 0건이면 ⚪(증거 없음 ≠ 이상), 라이브=OLD 면 🔴 배포 미반영.
- 🔀 **교차 출처 검증 `tests/odds-crosscheck.js`** (2026-08-31 신설, 네트워크 필요·아침 점검): 잠긴 경기마다 **프로바이더가 그 슬롯에 정말 라인이 없는지**를 `sports_odds` 원본으로 확인한다. 있으면 = **우리 매칭 실패** 🔴. **팀 이름 매칭을 쓰지 않는다** — 프로덕션 매처로 대조하면 정의상 같은 답이 나와 검사가 안 된다(같은 눈으로 두 번 보기). 대신 **킥오프 시각 ±15분 + 미소비 이벤트** 라는 이름과 독립인 신호만 본다. 8/28 약어 사고(맨시티·토트넘이 시즌 내내 잠김)를 이 검사는 즉시 잡는다.
  - ⚠️ 왜 필요했나: 자가검진·feed-check 는 **"잠김 = 프로바이더 공백"이라고 가정**하고 세기만 했다. 검사와 버그가 같은 착각을 공유하면 영원히 초록이다. **자기 정합성 검사는 내 착각을 못 잡는다 — 독립된 두 출처를 대조해야 잡힌다.** 새 검사를 만들 때 이 질문을 먼저 하라: *"이 검사는 내가 틀렸을 때 빨강이 되나, 아니면 나와 같이 틀리나?"*
...
- **상시 자가검진 (2026-07-19, 사장님 지시 "모든 문제는 저녁 보고 전에 잡아라"):** 검진 = **`node tests/daily-selfcheck.js` 한 방**(① 스케줄·오즈: 경기·실배당/잠금·가짜라인 ② 주문·결제·미청산: `sports-audit` C1~C9·미청산·최대노출·홀드 ③ 시세 라이브니스. 전부 읽기 전용, 돈/쓰기 0).
  - **시간당 무음 감시 = GitHub Action** `hourly-selfcheck.yml`(main, 매시 :25) — 2026-07-25 Claude 사용량 절감으로 이관. 침묵 게이트: 🔴면 사장님 이메일 → 아침/저녁 Claude 루틴이 원인 추적. (구 Claude 루틴 trig_01U7JZ9NTu8UJgYdjbkntxQd은 비활성 보관 — 재활성 = update_trigger enabled:true.) **발견의 층**.
  - **행동 계층 감시 = 합성 고객 봇** `synthetic-customer.yml`(main, 매일 14:10 UTC) — `tests/synthetic-customer.js`가 **사장님 테스트 계정(zbnyme@gmail.com, 2026-07-31 지정)**으로 3종 전부 리허설: 스포츠 $1 실베팅+**다음날 정산 완료 확인(36h+ 미정산=🔴)**→크립토 $2 왕복(멱등 프로브, **산 만큼만 매도 — 기존 보유 불가침**)→FX 0.01랏 왕복(주말 스킵)→cash_out 잠금 회귀→**Δ잔고==Σledger 센트 정합**. 안전 계약은 `synthetic-bot-pin.test.js`가 verify 게이트로 강제(금액 상한 $2/$1/0.01랏·매도 안전장치·시크릿 env 전용·fail-closed). 시크릿 `SYNTH_EMAIL`/`SYNTH_PASSWORD` 미설정이면 조용히 생략(활성화 = 리마인드-리스트 최상단).
...
- **[라이브 2026-09-21] 📱 모바일 FX 앱 = `fx-app.html` (RH형, 2단계 돈 연결 · 사장님 '교체해서 진행하자, 아직 고객이 없으니')**: 로그인(폰+FX)·가입(live)·스포츠/크립토 앱 서버 전환이 전부 여기로. 예전 앱 `trading.html` 은 보존(테스트 핀 다수, 고객 진입 경로 없음). 승인 "진행해. 1 켜, 2 A, 3 제거". 돈은 **서버 RPC 11개 허용목록(`RPC_ALLOW`)으로만** — fx_open(슬리피지 가드: 표시가 + FX 3핍/비FX 0.1%)·fx_modify·fx_close·fx_place_pending·**fx_modify_pending(신설 SQL, 수동 배포)**·fx_cancel_pending·pamm_join·pamm_leave·**app_transfer(2026-09-21 표 승인 "1 2 3 진행해")**·**fx_close_partial(2026-09-22 "4번" 승인·SQL 수동 배포 완료 — MT5 Close By Volume: 손익 = `fx_realized_pnl` 단일 함수 · 스왑 수량 비례 · settlements 1행(local_id = p_ref 멱등) · size 만 감소 · 잔여 ≥ 0.01 · 전량은 fx_close 로. 핀 `fx-close-partial.test.js`)** + 읽기 pamm_investor_report. 클라는 잔고·손익·포지션 행을 쓰지 않고 응답 뒤 서버를 재조회해 표시만. 한 주문 = 하나의 `local_id`(R-…) + busy 잠금 = 더블탭 1회. **풀 앱(2026-09-21~22)**: 입금/출금 = `requests` 행 insert(`AlpexaSync.pushRequest`, 승인 전 잔고 불변 · 출금 초과는 서버 `guard_withdraw_request` 거절 문구 그대로) · 이체 = `app_transfer` · Funding history(requests 읽기) · 차트 4h/1W/ALL 실봉 · 주식/지수 Twelve Data 실봉 · Security(auth.updateUser·전 기기 로그아웃) · Support(tel/mailto). 게이트 `trading-rh.test.js`(허용목록·호출 인자·더블탭·거절 문구·PAMM·입출금 insert 모양·requests 외 쓰기 0) + `fx-modify-pending.test.js`; `diagnose.js`·`diagnose-ui.js` DEPLOYED 에 포함. **새 RPC 추가 = 표 승인 먼저 → RPC_ALLOW + 핀.**
- **[보류 2026-08-18] ⚡ webtrade 사전 컴파일 (인브라우저 Babel 제거)**: 지금 고객은 터미널을 열 때마다 **@babel/standalone 3.0MB** 를 내려받아 **인라인 JSX 241KB** 를 그 자리에서 변환한다(콘솔 경고 `You are using the in-browser Babel transformer`). 미리 컴파일하면 그 3MB 가 통째로 사라져 첫 로딩이 확 빨라진다. 모바일 3종은 2026-07-31 에 이미 완료(`crypto-live` 799KB · `trading` 406KB · `manager` 735KB). **왜 보류인가**: 모바일은 처음부터 `src/*.jsx` 로 분리돼 있어 컴파일만 하면 됐지만, webtrade 는 241KB 가 **HTML 안에 통으로 박혀 있어** 먼저 밖으로 빼내야 한다. 리포에서 가장 큰 파일(322KB)이고 주문·포지션·잔고 화면이 전부 여기 있어 잘못 건드리면 터미널이 통째로 멈춘다. **착수 조건**: 로딩이 느리다는 얘기가 나오면. **작업량**: 하루. 순서 = ① 인라인 블록을 `src/webtrade-app.jsx` 로 추출 ② `tools/precompile-jsx.js` 대상에 추가 ③ HTML 은 `vendor/webtrade-compiled.js` 만 로드 + Babel CDN 제거 ④ `precompiled-fresh.test.js` 가 신선도·JSX 부활을 이미 감시하므로 대상만 추가 ⑤ verify + 헤드리스 스모크(주문·포지션 렌더)로 검증. 되돌리기는 커밋 하나 revert.
- **[라이브 2026-08-03] 💼 PAMM v1**: 유닛/NAV 방식(펀드=전용 FX계좌, fx 돈코어 무수정) — `pamm_core.sql` 배포·프로브 검증 완료(테이블 가동+미인증 거절), 데스크 `pamm-desk.html`(레전드 위젯 6종, confirmModal 관문). 돈 계약 P1~P7 = `pamm-core.test.js` verify 게이트. **포지션 정정(2026-09-08, 사장님 요청)**: `fx_admin_correct_position` — 관리자가 열린 FX 포지션의 side/open_price/size 를 정정(MT5 Manager Position Modify 상당). 잔고·원장·pnl 접근 0줄, 기록은 `admin_audit_log`(관리자 전용·삭제 불가) **한 곳** — 포지션 행(meta)엔 이력 없음(고객이 읽는 행). 데스크 ✎ 버튼 → 사유 필수 → confirmModal → RPC. 핀 `fx-position-correct.test.js`. **잔여**: ① 첫 펀드 개설 리허설(사장님) ② 고객 앱 PAMM 탭(참여/회수) ③ 합성 봇 PAMM 왕복 ④ EA 마스터 ⑤ IB 다단계 커미션(거래 기반만 — 바이너리/모집보상 금지 권고 전달됨).
- **[실고객 전 필수 체크리스트 2026-07-22 확정]** 실고객 얘기가 나오면 **먼저 리마인드**: ① CRON_SECRET 로테이션(교체 SQL 준비 완료 — 채팅에 새 시크릿 금지) ①b **sb_secret 키 재발급**(2026-07-24 채팅 노출·사장님 수용 — API Keys에서 New secret key 후 GitHub SUPABASE_KEY 교체) ② WELCOME_SECRET 교체 ③ **주식 실봉**(현재 과거봉 합성 — Finnhub 유료 ~$50/mo 또는 Polygon 주식 플랜 ~$29/mo 결정 후 반나절 작업. 현재가·체결은 실가격이라 돈 계산 무관, 차트 표시 신뢰 문제. 2026-07-31 SPACEX "가짜 절벽" 사건: 합성이 실가에 매끄럽게 붙게 chBase 실가 앵커로 임시봉합 완료 — 근본은 여전히 실봉 도입). 터미널 모바일 대응은 **불필요 확정**(2026-07-22 사장님 "우리는 앱버젼이 있잖아" — trading.html이 모바일).
