...
- **[실고객 전 필수 체크리스트 2026-07-22 확정]** 실고객 얘기가 나오면 **먼저 리마인드**: ① CRON_SECRET 로테이션(교체 SQL 준비 완료 — 채팅에 새 시크릿 금지) ①b **sb_secret 키 재발급**(2026-07-24 채팅 노출·사장님 수용 — API Keys에서 New secret key 후 GitHub SUPABASE_KEY 교체) ② WELCOME_SECRET 교체 ③ **주식 실봉**(현재 과거봉 합성 — Finnhub 유료 ~$50/mo 또는 Polygon 주식 플랜 ~$29/mo 결정 후 반나절 작업. 현재가·체결은 실가격이라 돈 계산 무관, 차트 표시 신뢰 문제). 터미널 모바일 대응은 **불필요 확정**(2026-07-22 사장님 "우리는 앱버젼이 있잖아" — trading.html이 모바일).
