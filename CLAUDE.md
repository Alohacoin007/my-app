...
- 🚫 **돈/잔고/보유/베팅을 `localStorage`에 "진실"로 저장하지 않는다.** 클라는 **매 로드마다 서버에서** 가져와 표시만 한다. 로컬 캐시는 (1)stale, (2)**계정간 누수**(한 브라우저 2계정→A 잔고가 B에 보임), (3)이중계산의 근원 — **$60/$80 이중차감·$90 누수가 전부 여기서 나왔다.** 돈 캐시를 "지키는" 코드(예: `cacheOwner`)를 추가하지 말고 **로컬에서 돈을 빼라.** (UI 설정·세션 토큰은 로컬 OK, 돈은 금지.)
  - 🔑 **세션 저장 = `sessionStorage`(탭별), `localStorage` 아님** (2026-06-29). 손님 Supabase 세션을 `sessionStorage`에 둠(`alpexa-sync.js`) → **탭마다 독립 세션 = 한 브라우저서 2계정 공존 가능.** 트레이드오프: **새 탭은 로그아웃 상태로 시작**(세션 미공유), 탭 내 새로고침은 유지, 탭/브라우저 닫으면 재로그인. (돈은 여전히 서버 전용 — 이건 세션 *토큰*의 저장 위치만 바꾼 것, #5의 "돈 금지"와 무관.) 어드민(백오피스)은 단일이라 격리 `localStorage` 키 유지.
