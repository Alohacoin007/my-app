...
- **상시 자가검진 (2026-07-19, 사장님 지시 "모든 문제는 저녁 보고 전에 잡아라"):** 검진 = **`node tests/daily-selfcheck.js` 한 방**(① 스케줄·오즈: 경기·실배당/잠금·가짜라인 ② 주문·결제·미청산: `sports-audit` C1~C9·미청산·최대노출·홀드 ③ 시세 라이브니스. 전부 읽기 전용, 돈/쓰기 0).
  ...
  - **행동 계층 감시 = 합성 고객 봇** `synthetic-customer.yml`(main, 매일 14:10 UTC) — `tests/synthetic-customer.js`가 봇 전용 계정으로 로그인→크립토 $2 왕복(멱등 프로브)→FX 0.01랏 왕복(주말 스킵)→cash_out 잠금 회귀→**Δ잔고==Σledger 센트 정합**. 안전 계약은 `synthetic-bot-pin.test.js`가 verify 게이트로 강제(금액 상한·시크릿 env 전용·fail-closed). 시크릿 `SYNTH_EMAIL`/`SYNTH_PASSWORD` 미설정이면 조용히 생략(활성화 3단계 = 리마인드-리스트 최상단).
