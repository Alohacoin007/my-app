...
- **[라이브 2026-09-21] 📱 모바일 FX 앱 = `fx-app.html` (RH형, 2단계 돈 연결 · 사장님 '교체해서 진행하자, 아직 고객이 없으니')**: 로그인(폰+FX)·가입(live)·스포츠/크립토 앱 서버 전환이 전부 여기로. 예전 앱 `trading.html` 은 보존(입출금·이체 화면, 테스트 핀 다수) — Deposit/Withdraw/Transfer 는 거기로 링크. 승인 "진행해. 1 켜, 2 A, 3 제거". 돈은 **서버 RPC 9개 허용목록(`RPC_ALLOW`)으로만** — fx_open(슬리피지 가드: 표시가 + FX 3핍/비FX 0.1%)·fx_modify·fx_close·fx_place_pending·**fx_modify_pending(신설 SQL, 수동 배포)**·fx_cancel_pending·pamm_join·pamm_leave + 읽기 pamm_investor_report. 클라는 잔고·손익·포지션 행을 쓰지 않고(insert/update 0) 응답 뒤 서버를 재조회해 표시만. 한 주문 = 하나의 `local_id`(R-…) + busy 잠금 = 더블탭 1회. 부분청산 RPC 없음 → 버튼 없음. Deposit/Withdraw/Transfer 는 기존 앱 링크. 게이트 `trading-rh.test.js`(허용목록·호출 인자·더블탭·거절 문구·PAMM) + `fx-modify-pending.test.js`. **새 RPC 추가 = 표 승인 먼저 → RPC_ALLOW + 핀.**
