...
- **[라이브 2026-08-03] 💼 PAMM v1**: 유닛/NAV 방식(펀드=전용 FX계좌, fx 돈코어 무수정) — `pamm_core.sql` 배포·프로브 검증 완료(테이블 가동+미인증 거절), 데스크 `pamm-desk.html`(레전드 위젯 6종, confirmModal 관문). 돈 계약 P1~P7 = `pamm-core.test.js` verify 게이트. **잔여**: ① 첫 펀드 개설 리허설(사장님) ② 고객 앱 PAMM 탭(참여/회수) ③ 합성 봇 PAMM 왕복 ④ EA 마스터 ⑤ IB 다단계 커미션(거래 기반만 — 바이너리/모집보상 금지 권고 전달됨).
