...
const DEPLOYED = ['crypto-live.html', 'sports-live.html', 'trading.html', 'fx-app.html', 'webtrade.html', 'fx.html', 'index.html',
...
const CHECKS = [
  ...
  { id: 'FEED-espn-spoof-headers', sev: 'HIGH',
    files: ['sports-games', 'sports-settle'].map((f) => `supabase/functions/${f}/index.ts`),
    re: /["']?(?:User-Agent|Referer)["']?\s*:\s*["'](?:Mozilla|https:\/\/(?:www\.)?espn)/i,
    why: 'ESPN 요청에 브라우저 위장 헤더(크롬 UA / Referer: espn.com)를 붙였다 — 이게 **403 을 만든다**. 2026-08-20 실측(tests/espn-transport-probe.js, 같은 러너·같은 IP·ms 차이): 헤더 없음 → 200 events=632/111/50, 위장 헤더 → 403/403/403. Akamai 는 헤더 없는 요청은 그냥 내주고, 크롬 UA 인데 TLS 지문이 크롬이 아닌 요청을 봇으로 본다. 위장은 통과가 아니라 탐지 신호. 2026-08-19 블랙아웃 때 "IP 차단이겠지"라는 가설로 이 헤더를 붙였다가 스포츠북이 하루 더 어두웠다 — ESPN 은 헤더 없이 부른다.' },
