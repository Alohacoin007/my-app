...
//    TL EURUSD · TR USDJPY · BL GBPUSD · BR USDCHF, all M30 candles (floatGeo tiles them edge-to-edge).
...
if (!/const initCharts=React\.useRef\(GRID\.map\(\(sym,i\)=>\(\{ id:i\+1, symbol:sym, tf:'M30', type:'candle', inds:\(cfg\.indicators\|\|\[\]\)\.slice\(\) \}\)\)\)\.current;/.test(src))
  bad('login must boot ONE chart per GRID symbol (4 × M30 candle), not a single maximized chart');
