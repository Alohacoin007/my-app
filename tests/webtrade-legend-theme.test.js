...
// Market Watch = MT5 light: up=네온 남색 #1330f0, down=네온 주황(빨강 쪽) #ff3d00 (2026-08-14 사장님 실관찰)
if (!/\.terminal\.light \.mwt \.au\{color:#1c9e2e !important\}\.terminal\.light \.mwt \.ad\{color:#ff3d00 !important\}/.test(src)) bad('Market Watch arrows must be green up / neon red-orange down');
if (!/\.terminal\.light \.mwt td\.tick-down\{color:#ff3d00 !important\}/.test(src)) bad('MT5 falling tick must be neon red-orange #ff3d00 (not pure red)');
...
if (!/\.terminal\.light \.mwtp \.sd\.sell \.pr\{color:#ff3d00\}/.test(src)) bad('Trading SELL price must match the Market Watch falling colour (#ff3d00)');
