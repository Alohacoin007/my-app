...
// INVARIANT LOCK: doFit re-pins the MT5 slim density (barSpacing 4 + rightOffset 15) on EVERY resize
// (not just the first), so a splitter-drag / window-resize can never roll the candle width back to fat.
if (!/const ts=chart\.current\.timeScale\(\);\s*\n\s*try\{ ts\.applyOptions\(\{ barSpacing:4, rightOffset:15 \}\); \}catch\(_\)\{\}/.test(src)) bad('doFit must re-pin barSpacing 4 + rightOffset 15 on every resize (invariant lock)');
...
console.log('🟢 PASS: default M1 chart snaps to the MT5 slim view (barSpacing 4 + rightOffset 15, scroll-to-realtime) the moment its (initially 0-wide) window gets a real size — candles show on load, no need to switch timeframe.');
