...
>
> **📍 현재 진행상황 (2026-06):** Resend엔 `alpexa-suisse.com`만 Verified 상태이고,
> **우리가 쓸 `alpexa-sports.com`은 아직 Resend에 없음** → **1~2단계를 실제로 해야 함**
> (Resend에 sports 도메인 Add → DNS 레코드 등록 → Verify). 그 다음 3→4→5→6→검증.
> 핵심 관문은 **4단계** — SMTP가 연결돼야 OTP가 Resend로 나간다(안 그러면 스팸).
