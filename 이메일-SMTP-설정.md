...
> - 발신 도메인: `alpexa-suisse.com`
> - 제안 발신 주소: `no-reply@alpexa-suisse.com`
>
> ⚠️ **코드 변경 없음.** signup의 `signInWithOtp`는 그대로 두면 된다 — SMTP를
> 연결하면 Supabase가 인증메일을 **자동으로** 그 SMTP로 보낸다.
>
> **📍 현재 진행상황 (2026-06):** Resend에 `alpexa-suisse.com` 도메인 **이미 Verified**
> (DNS/SPF/DKIM 완료) → **1~2단계 끝남.** 남은 건 **3단계(API키) → 4단계(Supabase SMTP
> 연결) → 5단계(한도) → 6단계(템플릿) → 검증.** 핵심 관문은 **4단계** — SMTP가 연결돼야
> OTP가 Resend로 나간다(안 그러면 여전히 Supabase 기본 발신자 = 스팸).
...
2. **Domains → Add Domain** → `alpexa-suisse.com` 입력.
...
4. 이 레코드들을 **도메인 DNS 관리화면**(가비아/Cloudflare/Route53 등 너가 alpexa-suisse.com 산 곳)에 추가.
...
| Type | Name | Value |
|---|---|---|
| TXT | `_dmarc.alpexa-suisse.com` | `v=DMARC1; p=none; rua=mailto:dmarc@alpexa-suisse.com; fo=1` |
...
| Sender email | `no-reply@alpexa-suisse.com` |
| Sender name | `Alpexa` |
...
- **Body:** 회사명/로고 + 코드 `{{ .Token }}` + "10분 내 만료" 안내 + 푸터에
  실제 회사명·주소·"문의 support@alpexa-suisse.com". (빈약한 본문/이미지 한 장은
  스팸 점수↑ — 텍스트와 균형 있게.)
